...
- Split frontend runtime, clock, round rendering, validator rendering, validator type/source helpers, and map rendering into smaller files.
...
2. Split `public/app/format.js` by domain:
   - dates and countdowns;
   - numeric/stake formatting;
   - address/hash display helpers.
3. Review map data and popup modules:
   - keep location grouping, clustering, popup HTML, and map view concerns separated;
   - add tests or visual checks around popup content where practical.
4. Re-audit Rust backend hotspots:
