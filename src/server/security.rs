...
fn add_common_headers(headers: &mut HeaderMap) {
    ...
    headers.insert(
        ...
        HeaderValue::from_static(
            "default-src 'self'; script-src 'self' https://unpkg.com; style-src 'self' 'unsafe-inline' https://unpkg.com; img-src 'self' data: blob: https://tiles.versatiles.org https://*.basemaps.cartocdn.com; connect-src 'self' https://tiles.versatiles.org https://*.basemaps.cartocdn.com; font-src 'self' data:; worker-src 'self' blob:; base-uri 'none'; frame-ancestors 'none'",
        ),
    );
    headers.insert(
