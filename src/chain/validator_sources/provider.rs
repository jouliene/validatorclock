...
#[derive(Clone)]
pub(super) struct JrpcValidatorSourceProvider {
    rpc: JrpcTransport,
    transport: Transport,
}
...
impl ValidatorSourceProvider {
    pub(super) fn new(chain_id: &str, endpoint: &str) -> Result<Self> {
        ...
        Ok(Self::Jrpc(JrpcValidatorSourceProvider { rpc, transport }))
    }
...
impl JrpcValidatorSourceProvider {
    async fn account_code_hash(&self, account_address: &str) -> Result<String> {
    ...
    async fn account_data(&self, account_address: &str) -> Result<Cell> {
        let state = self.transport.get_account_state(account_address).await?;
        let account = state
            .account()
            .with_context(|| format!("account `{account_address}` not found"))?;
