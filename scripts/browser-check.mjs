...
async function main() {
  ...
  await check(
    ...
    `(() => {
       ...
       tabs.find((tab) => !tab.hasAttribute("aria-current"))?.click();
       return { before };
     })()`,
  );
  await sleep(2500);
  ...
  await check(session, "original heading and network controls are restored", `document.querySelector('h1').textContent === 'VALIDATOR CLOCK' && document.querySelectorAll('#chainTabs button').length >= 2 && !document.querySelector('#chainTabs a')`);
  await check(session, "no added SEO content is visible", `!document.querySelector('.page-intro, .content-nav, #network-snapshot, .article-content, .dashboard-live')`);
  await check(session, "search metadata stays in the head", `JSON.parse(document.head.querySelector('script[type="application/ld+json"]').textContent)['@context'] === 'https://schema.org' && Boolean(document.head.querySelector('link[rel="canonical"]'))`);
  if (process.env.VALIDATORCLOCK_SCREENSHOTS) {
    mkdirSync(process.env.VALIDATORCLOCK_SCREENSHOTS, { recursive: true });
    for (const [name, width, height] of [["original-desktop", 1440, 1000], ["original-mobile", 390, 844]]) {
      ...
      writeFileSync(join(process.env.VALIDATORCLOCK_SCREENSHOTS, `${name}.png`), Buffer.from(capture.data, "base64"));
    }
  }
